...
foreach ($policy in $policies) {
    ...
    $mappingRow = [pscustomobject]@{
        TenantId                = Get-PrincipalValue -Row $policy -Primary 'TenantId'
        ...
        SuggestedRole2Id        = if ($roleByName.ContainsKey($role2)) { $roleByName[$role2].id } else { '' }
        SuggestedRole3Id        = if ($roleByName.ContainsKey($role3)) { $roleByName[$role3].id } else { '' }
...
$commandLines.Add('# Vault-scoped assignments do not survive a resource move. Regenerate after moving a vault.')
$commandLines.Add('# Prefer New-KeyVaultRbacStagingPlan.ps1 and Set-KeyVaultRbacAssignments.ps1 for idempotent staging.')
$commandLines.Add('# Static commands remain commented unless Resolve-KeyVaultRbacMapping.ps1 is run with -EmitActiveCommands.')
$commandLines.Add('')

foreach ($row in $mappingRows) {
    $canEmitCommand = $EmitActiveCommands -and ($CommandStatuses -contains $row.MappingStatus) -and $row.SuggestedRole1Id -and (-not $row.AlreadyAssigned)
    $principal = Escape-QuotedString $row.PrincipalId
    $role = Escape-QuotedString $row.SuggestedRole1
    $roleId = Escape-QuotedString $row.SuggestedRole1Id
    $scope = Escape-QuotedString $row.VaultId

    if ($canEmitCommand) {
        $commandLines.Add("New-AzRoleAssignment -ObjectId '$principal' -RoleDefinitionId '$roleId' -Scope '$scope' # $role")
    }
    else {
